  ...
  return {
    ...
    controller: ('sirAccordionCtrl',['$scope',function ($scope) {
      $scope.config = {
        ...
        menuLink: $scope.config.menuLink || ''
      };
    }]),
    link: function(scope,element) {
      ...
      */
      var itemRegen = function(collection, parentIndex, currentIndex, level) {
		...
		var iconImg = '<img ng-src="'+collection[currentIndex].small_icon+'">';
        uniqueIndex = level ? String(parentIndex) + '-' + String(currentIndex + 1) : String(currentIndex + 1);

		var cutomTilte = collection[currentIndex].title.toLowerCase();

		if(parentIndex<1){
			if(collection[currentIndex].subCollection!=null){
				cutomTilte = iconImg+collection[currentIndex].title.toLowerCase()+"<span></span>";
		   }else{
				cutomTilte = iconImg+collection[currentIndex].title.toLowerCase();
		   }

		}else{
			if(scope.config.menuLink!=''){
				var catLink = scope.config.menuLink+"/"+collection[currentIndex].category_id;
				cutomTilte = '<a href="'+catLink+'" class="lev1"  menu-close>'+collection[currentIndex].title.toLowerCase()+"</a>";
			}else{
				cutomTilte = '<a href="#" class="lev1" menu-close>'+collection[currentIndex].title.toLowerCase()+"</a>";
			}
		}

		header = scope.config.beforeHeader +cutomTilte+ scope.config.afterHeader;

        domContents.push(uniqueIndex);

		//console.log(parentIndex+" : "+currentIndex+" : "+collection[0].subCollection.toSource());
        item =
