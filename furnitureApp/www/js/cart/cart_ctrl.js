...
.controller('CartCtrl', function($scope,$rootScope,$ionicLoading,alertmsgService,$location,eCart)
{
	...
	$rootScope.cartRefresh  = function(){
		...
		$scope.categoryName = $rootScope.cat_Name;
		$scope.categoryId = $rootScope.cat_id;
	...
	$scope.showProductDetail = function(productId,categId,caegName){
		console.log(productId);
		console.log(categId);
		//console.log(caegName);
		$location.path('app/productdetail/'+productId+'/'+categId+'/'+caegName);
	 }
...
.controller('CartDeliveryCtrl', function($scope,$http,$rootScope,$ionicModal,$location,usersService,alertmsgService,eCart) {
    ...
    $scope.removeAddress=function(value){
		//$scope.billAddresses.splice(value, 1);
		$scope.billAddresses.splice($scope.billAddresses.indexOf(value),1);
	...
	//shipping address
$scope.removeShipAddress=function(value){
		//$scope.billAddresses.splice(value, 1);
		$scope.shipAddresses.splice($scope.shipAddresses.indexOf(value),1);
...
'use strict';
		...
		$scope.grossTotal =(parseFloat(parseFloat($scope.subTotal)+parseFloat($rootScope.shppingMethodData.cost))).toFixed(2);
	
          
          console.log($scope.subTotal,$rootScope.shppingMethodData.cost,$scope.grossTotal);
